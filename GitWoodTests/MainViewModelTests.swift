...
class MainViewModelTests: XCTestCase {
    ...
    func testCellInfoFor() {
        ...
        let testCellInfo = CellInfoModel(name: "ASPNETSelfCreatedTokenAuthExample", detailed: "Example of how to protect an ASP.NET Core (1.0.1) Web API using simple self-created JWT bearer tokens.", avatarUrl: URL(string: "https://avatars3.githubusercontent.com/u/7505593?v=4")!, isFavorited: false)
        
        let cell = mainViewModel.cellInfoFor(indexPath: IndexPath(item: 0, section: 0))
       ...
       let single = mainViewModel.loadTrendingRepos(period: .LastMonth).subscribeOn(scheduler)
        ...
        case .completed(elements: let repos):
             XCTAssertEqual(mainViewModel.count, 30)
              XCTAssertEqual(repos.first!, true)
            break
